...
export default function App() {
  ...
  useEffect(() => {
    const savedTasks = localStorage.getItem("foquinha_tasks");
    const savedGoals = localStorage.getItem("foquinha_goals");
  ...
  useEffect(() => {
    localStorage.setItem("foquinha_tasks", JSON.stringify(tasks));
  }, [tasks]);

  useEffect(() => {
    localStorage.setItem("foquinha_goals", JSON.stringify(goals));
  }, [goals]);
  ...
  const todayTasks = useMemo(() => tasks.filter((t) => !t.done), [tasks]);

  const whatsappSummary = useMemo(() => {
    const lines = ["🗓️ *Agenda Inteligente*", "", "*Hoje*"];
    const current = tasks.filter((t) => !t.done);

    if (!current.length) {
      lines.push("Tudo concluído por aqui ✅");
    } else {
      current.forEach((task, i) => {
        const emoji = task.priority === "alta" ? "🔥" : "✅";
        lines.push(`${i + 1}. ${emoji} ${task.text} — ${task.date}`);
      });
    }

    lines.push("", "*Próximos passos*");
    current.slice(0, 5).forEach((task, i) => {
      lines.push(`${i + 1}. ${task.text} — ${task.date}`);
    });
  ...
  function addTask() {
    ...
    const newTask = {
      ...
      text: input.trim(),
      date: "Hoje, 20:00",
      priority: input.toLowerCase().includes("urgente") ? "alta" : "média",
      category: "rotina",
      done: false,
    };
            ...
            setGoals((oldGoals) =>
              oldGoals.map((goal) =>
                goal.id === 2
                  ? {
                      ...goal,
                      current: Math.min(goal.target, goal.current + 1),
                    }
                  : goal
              )
            );
      ...
      <div style={styles.container}>
        ...
        <div style={styles.card}>
          <div style={styles.sectionTop}>
            <div style={styles.sectionTitle}>Adicionar rápido</div>
          ...
          <div style={styles.inputRow}>
            <input
              style={styles.input}
              placeholder="amanhã 14h estudar biologia urgente"
              value={input}
              onChange={(e) => setInput(e.target.value)}
            />
            <button style={styles.plusButton} onClick={addTask}>
              +
            </button>
          </div>

          <div style={styles.suggestions}>
            {["hoje 19h revisar redação", "amanhã 21h treino", "domingo 18h planejar semana"].map(
              (item) => (
                <button key={item} style={styles.suggestion} onClick={() => setInput(item)}>
                  {item}
                </button>
              )
            )}
            ...
            <div style={styles.taskList}>
              {todayTasks.map((task) => (
                <div key={task.id} style={styles.taskItem}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={styles.taskTitle}>{task.text}</div>
                    <div style={styles.taskMeta}>
                      {task.priority === "alta" ? "🔥 alta" : "✅ média"} · {task.category} · {task.date}
                    </div>
                  </div>
